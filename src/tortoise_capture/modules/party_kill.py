...
class PartyKill(BaseModule):
    ...
    def decode(self, pkt: Packet, ctx: DecodeContext) -> Iterator[Event]:
        ...
        killer = r.u64("killer")
        victim = r.u64("victim")
        yield self.event(pkt, "party_kill", guid=victim, entry=guid_entry(victim),
                         victim_type=guid_type(victim), killer_guid=killer,
                         killer_entry=guid_entry(killer))
    ...
    def sql_rows(self, ev: Event, ctx: SqlContext) -> Iterator[Row]:
        yield Row(_TABLE.name, {"capture": ctx.capture_id, "t": ev.packet.t,
                                ...
                                "victim_guid": ev.data["guid"], "entry": ev.data["entry"]})
