...
| `SMSG_GAMEOBJECT_QUERY_RESPONSE` (0x5F) | `uint32 entry` + `uint32 type` + `uint32 displayId` + CString name + 4×`uint8(0)` (name2–4 and one more) + 24×`uint32` data (`QueryHandler.cpp:250`). The data block is `GameObjectInfo::raw` appended as `uint32[24]` — `ByteBuffer::append<T>` writes `cnt * sizeof(T)` bytes, so 96, not 24 — and its meaning depends on `type`, exactly as `gameobject_template.data0..23`. No faction, flags or size: those are UpdateFields. `entry | 0x80000000` with no body is "unknown" (also sent by `World::SendGameObjectStatsInvalidate`). |
...
| `SMSG_SPELL_GO` | packGUID (cast item or caster) + packGUID (caster, possibly an **empty** packguid — the spell 10444 hack, `Spell.cpp:4672`) + `uint32 spellId` + `uint16 castFlags`, then `WriteSpellGoTargets` (`Spell.cpp:4758`): `uint8 hitCount` + hitCount × **raw** `uint64`, `uint8 missCount` + missCount × (**raw** `uint64` + `uint8 missCondition` **+ `uint8 reflectResult` iff the condition is `SPELL_MISS_REFLECT` (11)**, `Spell.cpp:4798`). Then `SpellCastTargets::write` (`Spell.cpp:228`) **unconditionally**: `uint16 targetMask`, then one packGUID (or a lone `0` byte) for UNIT/OBJECT/CORPSE by the source's own if/else-if priority — a mask with two of them writes **one** guid, not two — then a packGUID for ITEM/TRADE_ITEM, 3×float for SOURCE_LOCATION (0x20), 3×float for DEST_LOCATION (0x40), CString for STRING. Finally, iff `CAST_FLAG_AMMO` (0x20): `uint32 ammoDisplayId` + `uint32 ammoInventoryType` (`Spell.cpp:4754`) — **both uint32**. The reflect byte is the trap: read misses as fixed-width and every field after them decodes as garbage. Measured on two real captures: 871 casts, 0 reflected misses, so that branch is covered by synthetic tests only. |
...
| `SMSG_SPLINE_SET_*_SPEED` (0x2FE-0x303) / `SMSG_SPLINE_MOVE_SET_*_MODE` (0x30D-0x30E) | speed: packGUID + `float speed` (`MovementPacketSender.cpp:118-131`); mode: packGUID alone (`Unit.cpp:11487`). The spline forms are for units the server moves; a player-controlled unit gets `SMSG_FORCE_*`/`MSG_MOVE_SET_*` instead (`MovementPacketSender.cpp:25`). The float is the **speed, not the rate** -- `rate x baseMoveSpeed`, so Ralthas's run is 8.0 = 7.0 x 1.14286. Most are changes *from* the template's speed (a slow landing and falling off), and `MoveSplineInit.cpp:173-187` sends a charge's own velocity and then restores the real one -- the speed a creature was created with is in its CREATE movement block. |
| `MSG_MOVE_*` (22 opcodes) | **C2S:** a bare `MovementInfo`, no guid (`MovementHandler.cpp:320`, layout `Object.cpp:64` — `uint32 moveFlags`, `uint32 ctime`, 4×float pos, transport/swim extras, **unconditional `uint32 fallTime`**, jump/spline extras). The mover is not on the wire because the server takes it from the session (`MovementHandler.cpp:304`), which makes a client-sent move the recording player *by construction*. **S2C: a different layout for a different body** — the server relays another player's move as packGUID + the same `MovementInfo` (`MovementHandler.cpp:456`). `MSG_` opcodes share one number across both directions; branch on direction, never on the number alone. The authoritative list of the 22 is whatever `Opcodes.cpp` routes to `HandleMovementOpcodes`. |
